...
class KhachHang extends MY_Controller
{
	...
	public function index()
	{
		...
		$data['list'] = $this->Model_KhachHang->getById($this->session->userdata('khachhang'));
		$data['khachhang'] = $this->Model_KhachHang->getByTaiKhoan($this->session->userdata('khachhang'));
		// var_dump($data);die;
	...
	public function Update()
	{
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			...
			$this->Model_KhachHang->update($tenkhachhang, $sodienthoai, $diachi, $email, $taikhoan, $avatar);
	...
	public function upload_image()
	{
		...
		if (!$this->upload->do_upload('avatar')) {
			...
			echo json_encode(array('status' => true, 'file_path' => $file_path));
			// return json_encode(array('status' => true, 'file_path' => $file_path));
		}
	}
}
