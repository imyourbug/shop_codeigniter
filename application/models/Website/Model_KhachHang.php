...
class Model_KhachHang extends CI_Model
{
	...
	public function getById($taikhoan)
	{
		$sql = "SELECT donhang.*, khachhang.* FROM donhang, khachhang WHERE donhang.MaKhachHang = khachhang.MaKhachHang AND khachhang.TaiKhoan = ?";
		$result = $this->db->query($sql, array($taikhoan));
		return $result->result_array();
	}

	public function getByTaiKhoan($taikhoan)
	{
	...
	public function update($tenkhachhang, $sodienthoai, $diachi, $email, $taikhoan, $avatar)
	{
		$sql = "UPDATE khachhang SET TenKhachHang=?,SoDienThoai=?,DiaChi=?,Email=?,avatar=? WHERE TaiKhoan=?";
		$result = $this->db->query($sql, array($tenkhachhang, $sodienthoai, $diachi, $email, $avatar, $taikhoan));
		return $result;
	}
